export default async function handler(request, response) {
    const { lat, lon, problem, vehicleBrand } = request.query;
    ...
    console.log(`LOG: Searching workshops for problem: ${problem}, vehicle: ${vehicleBrand}`);
    ...
    try {
        ...
        if (vehicleBrand && vehicleBrand !== 'Nicht angegeben') {
            searchQueries.push({
                ...
                type: 'dealership',
                radius: 10000 // Größerer Radius für Vertragshändler
            });
        }
        ...
        const allResults = await Promise.all(
            searchQueries.map(async (searchConfig) => {
                ...
                if (searchData.status === "OK") {
                    ...
                    return searchData.results.slice(0, 3).map(place => ({
                        ...place,
        ...
        const workshopsWithDetails = (await Promise.all(workshopDetailsPromises))
            ...
            .slice(0, 8); // Maximal 8 Werkstätten
